...
try{
    ...
    private fun updateStorage(uri: String,storage: UriStorage){
        ...
        imageDisplaySurface.setImageBitmap(Bitmap.createScaledBitmap(BitmapFactory.decodeFileDescriptor(contentResolver.openFileDescriptor(Uri.parse(uri),"r")?.fileDescriptor),imageDisplaySurface.width,imageDisplaySurface.height,true))
